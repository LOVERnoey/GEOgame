...
$guide_query = "SELECT g.guide_id, g.guidename, g.guideprofile, g.guidedescription, g.guideimage, g.guiderating, u.user_name FROM guide g JOIN users u ON g.user_id = u.id WHERE g.guide_id = ?";
...
<body>
    ...
    <div class="container">
        ...
        <form action="delete_guide.php" method="POST" onsubmit="return confirm('Are you sure to delete this guide?');">
            <input type="hidden" name="guide_id" value="<?= htmlspecialchars($guide['guide_id']) ?>">
            <button type="submit" class="btn btn-danger" style="float: right;">Delete Guide</button>
        </form>


        <?php if (!empty($guide['guideprofile'])): ?>
            <img src="../uploads/<?php echo htmlspecialchars($guide['guideprofile']); ?>" alt="Guide Profile Image"
                class="guide-image" style="width: 150px; height: 150px; border-radius: 10%;">
        <?php endif; ?>
        ...
        <?php if (!empty($guide['guideimage'])): ?>
            <img src="../uploads/<?php echo htmlspecialchars($guide['guideimage']); ?>" alt="Guide Image"
                class="guide-image">
        <?php endif; ?>
